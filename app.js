...
function renderizarListaIdeas(ideas) {
    ...
    if (ideas.length === 0) {
        contenedor.innerHTML = `<p style="text-align: center; color: var(--text-muted); padding: 20px; font-size: 12px;">No hay propuestas con este filtro.</p>`;
        return;
    }
        ...
        if (esAdmin || esPropio) {
            htmlBotonesAccion = `
                <div style="display: flex; gap: 8px; margin-top: 10px; padding-top: 8px; border-top: 1px solid var(--border);">
                    <button onclick="cambiarEstadoIdea('${k.id}', '${k.estado === 'ABIERTO' ? 'CERRADO' : 'ABIERTO'}')" style="background: var(--primary-light); color: white; padding: 5px 10px; font-size: 10px; border-radius: 6px; width: auto;">
                        ${k.estado === 'ABIERTO' ? 'Marcar Cerrado' : 'Marcar Abierto'}
                    </button>
                    <button onclick="eliminarIdea('${k.id}')" style="background: var(--danger); color: white; padding: 5px 10px; font-size: 10px; border-radius: 6px; width: auto;">Eliminar</button>
                </div>
            `;
        }
...
function renderizarPistaPilotos(ranking) {
    ...
    if (ranking.length === 0) {
        contenedor.innerHTML = `<p style="text-align: center; color: var(--text-muted); padding: 20px; font-size: 12px;">Sin actividad en pista.</p>`;
        return;
    }
    ...
    ranking.forEach((piloto, index) => {
        ...
        card.innerHTML = `
            ...
            <div class="racer-track-line">
                <div class="racer-car-icon" style="left: ${porcentaje}%;">►</div>
